...
pub fn resolve_file(
    ...
    while let Some(m) = matches.next() {
        ...
        let Some(caller_name) = function_name(fn_node, source) else {
            continue;
        };
        let caller_line = fn_node.start_position().row as u32 + 1;
        let caller_id = symbol_id(relative_path, NodeKind::Function, &caller_name, caller_line);

        let Some(target_id) = find_rust_function(rust_files, store, callee_name) else {
            continue;
        };
